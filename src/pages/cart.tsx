...
function Cart () {
  const { cartItems, removeItem, decreaseItemQuantity, increaseItemQuantity, clearCart } = useShoppingCart()
  return (
    <>
      ...
      <section className='pt-10 pb-20'>
        <div className='relative pt-6 pb-14'>
          <h2 className='text-center  text-3xl main-text'>My Shopping Cart</h2>
          {cartItems.length > 0 && <button type='button' onClick={() => clearCart()} className='absolute top-20 right-6 md:right-16 underline underline-offset-4'>Clear Cart</button>}
        </div>
        {!cartItems.length ? (
                      ...
                      <div className='flex flex-col gap-y-4 md:gap-y-8 '>
                        ...
                        <div className='flex flex-col  md:flex-row md:gap-6 md:items-center'>
                          ...
                          <div className='flex gap-x-5 pt-4 md:pt-0'>
                            <button type='button' onClick={() => decreaseItemQuantity(item.id, item.size , item.color)} ><RemoveCircleOutlineRoundedIcon className='!text-3xl cursor-pointer text-blue-500' /></button>
